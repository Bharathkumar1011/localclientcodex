...
export default function ContactManagementOtherFields() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ["leads", "stage"] });
  ...
  const filteredRows = useMemo(() => {
    ...
    // ✅ persisted value only (server) - used for Blank/Filled filter so rows don't disappear while typing
    ...
    switch (fieldFilter) {
        ...
        return row.website ?? "";
        case "revenueInrCr":
        return row.revenueInrCr;
        case "ebitdaInrCr":
        return row.ebitdaInrCr;
        case "patInrCr":
        return row.patInrCr;
        default:
        return "";
    ...
    const getLiveValue = (row: OtherFieldsRow) => {
      const d = drafts[row.leadId];
      switch (fieldFilter) {
        case "sector":
          return d?.sector ?? row.sector ?? "";
        case "leadSource":
          return d?.leadSource ?? row.leadSource ?? "";
        case "website":
          return d?.website ?? row.website ?? "";
        case "revenueInrCr":
          return d?.revenueInrCr ?? row.revenueInrCr;
        case "ebitdaInrCr":
          return d?.ebitdaInrCr ?? row.ebitdaInrCr;
        case "patInrCr":
          return d?.patInrCr ?? row.patInrCr;
        default:
          return "";
      }
    };
    ...
    return (items || []).filter((row) => {
      ...
      if (partnerFilter !== "all" && (row.partnerId || "") !== partnerFilter) return false;
        ...
        if (statusFilter !== "all") {
        // ✅ IMPORTANT: filter must use ONLY persisted server value
        // so Blank rows stay visible while user types and move only after Save+refetch
  ...
  const headerCounts = useMemo(() => {
    ...
    for (const row of filteredRows) {
      ...
      if (isFilledText(sector)) counts.sector.f += 1;
      else counts.sector.b += 1;

      if (isFilledText(leadSource)) counts.leadSource.f += 1;
      else counts.leadSource.b += 1;

      if (isFilledText(website)) counts.website.f += 1;
      else counts.website.b += 1;

      if (isFilledNumber(revenue)) counts.revenue.f += 1;
      else counts.revenue.b += 1;

      if (isFilledNumber(ebitda)) counts.ebitda.f += 1;
      else counts.ebitda.b += 1;

      if (isFilledNumber(pat)) counts.pat.f += 1;
      else counts.pat.b += 1;
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-2">
            ...
            <Select value={statusFilter} onValueChange={(v: any) => setStatusFilter(v)}>
              ...
              <SelectContent className="bg-gray-50">
                ...
                <SelectItem value="not_blank">Filled</SelectItem>
              </SelectContent>
            </Select>
            ...
            <table className="w-full text-sm">
             <thead className="sticky top-0 z-20 bg-white shadow-sm">
                <tr className="border-b bg-white">
                  ...
                  <th className="px-3 py-2 text-left w-[180px] bg-white">
                    Sector{" "}
                    <span className="text-muted-foreground font-normal">
                      (F {headerCounts.sector.f} | B {headerCounts.sector.b})
                    </span>
                  </th>
                  <th className="px-3 py-2 text-left w-[180px] bg-white">
                    Lead Source{" "}
                    <span className="text-muted-foreground font-normal">
                      (F {headerCounts.leadSource.f} | B {headerCounts.leadSource.b})
                    </span>
                  </th>
                  <th className="px-3 py-2 text-left w-[260px] bg-white">
                    Website{" "}
                    <span className="text-muted-foreground font-normal">
                      (F {headerCounts.website.f} | B {headerCounts.website.b})
                    </span>
                  </th>
                  <th className="px-3 py-2 text-left w-[260px] bg-white">
                    Financial Info{" "}
                    <span className="text-muted-foreground font-normal">
                      (R F {headerCounts.revenue.f} | B {headerCounts.revenue.b} · E F {headerCounts.ebitda.f} | B{" "}
                      {headerCounts.ebitda.b} · P F {headerCounts.pat.f} | B {headerCounts.pat.b})
                    </span>
                  </th>
                  <th className="px-3 py-2 text-center w-[120px] bg-white">Save</th>
                </tr>
              </thead>
