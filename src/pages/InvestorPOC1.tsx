...
// ✅ CONFIG
const SLOT_INDEX = 0; 
const PAGE_TITLE = "Investor POC 1 (Primary)";
...
export default function InvestorPOC1() {
...
const { data, isLoading, isFetching } = useQuery<POCResponse>({
  ...
  // 2. Initialize Drafts (Optional, ensures inputs are controlled)
  useEffect(() => {
  ...
  // Persist selected stages to sessionStorage so it survives page reloads (but not shared across tabs)
  useEffect(() => {
  try {
  ...
  const filteredRows = useMemo(() => {
    ...
    return items.filter((r) => {
      ...
      // Status Filter (Blank/Not Blank)
      if (statusFilter !== "all") {
        // IMPORTANT: We filter based on the ORIGINAL server value so rows don't vanish while typing
  ...
  // 4. Dynamic Header Counts (Calculates "Filled vs Blank" stats in real-time)
  const headerCounts = useMemo(() => {
    ...
    for (const r of filteredRows) {
      ...
      // Use DRAFT value if available (updates stats as you type), else Server value
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["investor-contact-metrics"] }); // Update main dashboard
    },
    onError: () => {
  ...
  // Handler for input changes in the table (updates drafts state)
...
const toggleStage = (stage: InvestorStageFilter) => {
  setSelectedStages((prev) => {
    ...
    // add and keep the original stage order
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Manage and clean primary contact data for investors.</p>
      </div>
              ...
              <table className="w-full text-sm border-t">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    ...
                    <th className="text-right p-3 font-medium w-[80px] align-top pt-3">Action</th>                  </tr>
                </thead>
                <tbody className="divide-y">
                  {filteredRows.map(row => {
                    const draft = drafts[row.investorId] || {};
                    
                    // Logic to highlight "Save" button if modified
                    const isModified = 
